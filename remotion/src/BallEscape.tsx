	...
	const simulation = useMemo(() => {
		...
		for (let f = 0; f < durationInFrames; f++) {
			if (ringsEscaped < NUM_RINGS && f < 1800) {
				...
				// Without substeps, a fast ball can jump over a ring between frames and
				// the next frame starts with c > 0 (ball already outside), missing the collision.
				let remainingT = 1.0;
				...
				for (let substep = 0; substep < MAX_SUBSTEPS && remainingT > 1e-9; substep++) {
					...
					// currentRingAngles holds the end-of-frame angle; subtract what remains
					...
					if (angleDiff < GAP_SIZE / 2) {
						...
						ballVX = nx * (speed + 1.5);
						ballVY = ny * (speed + 1.5);
						// Place ball just outside the escaped ring so c > 0 won't re-trigger it,
						// but well inside the next ring (40 px away) so the substep loop catches it
						ballX = hitX + nx * 0.01;
						ballY = hitY + ny * 0.01;
					} else {
						...
						// Place ball just inside the ring so the reflected velocity is used next substep
						ballX = hitX - nx * 0.01;
						ballY = hitY - ny * 0.01;
					}
				}
			...
			<div style={{ position: 'absolute', left: centerX, top: centerY, width: 0, height: 0, display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 2, opacity: showResult ? 0.2 : 1 }}>
				{[...ringRadii].map((_, idx) => idx).reverse().map((i) => {
					...
					if (deathF !== -1 && frame >= deathF) {
						...
						return <div key={i} style={{ position: 'absolute', width: radius * 2, height: radius * 2, border: `${RING_THICKNESS}px solid hsl(${i * 18}, 90%, 60%)`, borderRadius: '50%', opacity: interpolate(frame, [deathF, deathF + 15], [1, 0]), transform: `rotate(${angle + Math.PI / 2}rad) scale(${interpolate(frame, [deathF, deathF + 15], [1, 1.2])})`, filter: 'blur(5px)' }} />;
					}
					const gapDeg = (GAP_SIZE * 180) / Math.PI;
					return (
						<div key={i} style={{ position: 'absolute', width: radius * 2, height: radius * 2, borderRadius: '50%', border: `${RING_THICKNESS}px solid transparent`, backgroundImage: `conic-gradient(transparent 0deg, transparent ${gapDeg / 2}deg, hsl(${i * 18}, 90%, 60%) ${gapDeg / 2}deg, hsl(${i * 18}, 90%, 60%) ${360 - gapDeg / 2}deg, transparent ${360 - gapDeg / 2}deg, transparent 360deg)`, backgroundOrigin: 'border-box', backgroundClip: 'border-box', transform: `rotate(${angle + Math.PI / 2}rad)`, boxShadow: `0 0 30px hsl(${i * 18}, 90%, 60%)44`, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
							<div style={{ width: (radius - RING_THICKNESS) * 2, height: (radius - RING_THICKNESS) * 2, backgroundColor: 'black', borderRadius: '50%' }} />
						</div>
					);
				})}
				<div style={{ position: 'absolute', left: ballX - BALL_RADIUS, top: ballY - BALL_RADIUS, width: BALL_RADIUS * 2, height: BALL_RADIUS * 2, backgroundColor: 'white', borderRadius: '50%', boxShadow: '0 0 40px white', zIndex: 100 }} />
			</div>
